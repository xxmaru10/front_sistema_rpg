...
| Estresse Customizável (Story 25) | `StressBox = { value, checked }` substitui `boolean[]`. `normalizeStressTrack` garante retrocompatibilidade. GM edita valor inline via input na `CharacterVitality`. Evento `STRESS_BOX_VALUE_CHANGED` persiste o valor. | 2026-04-06 |
| Som de Impacto de Dados (Story 25) | `_hasImpacted` flag por dado em `dicePhysics.ts` dispara `onFirstFloorHit` callback uma única vez. Som acionado no primeiro impacto físico, não no clique. Safety timeout de 15s garante reset de `isRolling`. | 2026-04-06 |
| Impulsos no Combat Card (Story 25) | `character.impulses: number` (nunca negativo via `Math.max(0,...)`). Apenas GM adiciona (botão `ArrowDown`); qualquer jogador remove clicando no indicador. Double-click pattern (pendingRemove) para remoção de personagem da arena — elimina `confirm()` nativo. `actorUserId` normalizado no `useCombatCard`. | 2026-04-06 |
